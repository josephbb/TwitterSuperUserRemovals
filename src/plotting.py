...
def plot_posterior_and_change(posterior_location,
    ...
    #Plot change
    t=(np.exp(idata.posterior_predictive.mu_hat)[:,:,-1] - np.exp(idata.posterior_predictive.mu_hat)[:,:,29])
    t/=np.exp(idata.posterior_predictive.mu_hat)[:,:,29]
    plt.sca(ax[1])
    sns.distplot(100*np.array(t).flatten(),kde=True, ax=ax[1])
    plt.xlabel('Percent change in posts per day')
    plt.savefig('./output/figures/posteriors/'+posterior_location.split('/')[-1].split('.')[0]+'.png',dpi=300)
    
    
def plot_change_by_group(group, ax, color, percentile_list=[2.5, 25, 50, 75, 97.5],xlab=False):
    ...
    plt.xlim(xlim)
    #plt.plot(xlim, [np.median(np.mean(group['results'].apply(np.array))), 
    #                np.median(np.mean(group['results'].apply(np.array)))],color='k')
    #plt.fill_between(xlim, [np.percentile(np.mean(group['results'].apply(np.array)),3), 
     #               np.percentile(np.mean(group['results'].apply(np.array)),3)],
      #                [np.percentile(np.mean(group['results'].apply(np.array)),97), 
       #             np.percentile(np.mean(group['results'].apply(np.array)),97)],
        #              color=color,alpha=.5)
